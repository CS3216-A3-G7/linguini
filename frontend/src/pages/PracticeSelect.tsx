...
export function PracticeSelect() {
  ...
  useEffect(() => {
    ...
    let settled = false;
    let elapsed = false;
    const arm = () => { if (!cancelled && settled && elapsed) setReady(true); };
    const timer = setTimeout(() => { elapsed = true; arm(); }, 400);
    getActivePractice()
      .then(detail => {
        if (cancelled) return;
        if (detail) { navigate(`/practice/sessions/${detail.session.id}/analysis`, { replace: true }); return; }
        settled = true; arm();
      })
      .catch(() => { settled = true; arm(); });
    return () => { cancelled = true; clearTimeout(timer); };
  }, [navigate]);
  const start = async (asset: string) => {
    if (!ready || busy.current || !activeProfile) return;
  ...
  return <div className="stack practice-select">
    ...
    <div className="dashed-capture">
      ...
      <ImageUpload compact disabled={!ready || selected || starting || !activeProfile} cameraEnabled={learner.cameraOn}
        onBusyChange={setUploading} onUploaded={image => void start(image.id)} />
    </div>
    {starting ? <p role="status">Starting analysis...</p> : null}
    {!ready ? <p role="status">Checking for a session in progress...</p> : null}
    ...
    <div className="grid-2">
      {scenes.map(scene => <button key={scene.id} type="button" className="scene-pick"
        disabled={!ready || selected || uploading || starting || !activeProfile} aria-label={`Choose ${scene.title}`} onClick={() => void start(scene.mediaAssetId)}>
