# Product Hunt launch plan: Linguini

**Proposed launch date: Wednesday 14 October 2026.**
Product Hunt launches go live at 12:01 am Pacific Time. On that date the Pacific coast is on daylight time (PDT, UTC−7), so launch is at **3:01 pm Singapore time on Wednesday 14 October** (07:01 UTC). The Product Hunt day ends at 11:59 pm PDT, which is **2:59 pm SGT on Thursday 15 October**. Plan for 24 hours of launch, running from Wednesday afternoon to Thursday afternoon in Singapore.

> **Daylight saving check:** US daylight time ends on Sunday 1 November 2026. From then on, Pacific is PST (UTC−8), and 12:01 am PT becomes **4:01 pm SGT**. Singapore has no daylight saving. If the date moves past 1 November, change every SGT time in this plan by +1 hour.

Why this date: it gives two weeks to clear the must-fix list in `readiness.md` and stays within Product Hunt's 30-day scheduling window. It is also a mid-week day, when Product Hunt gets its normal traffic. Before you schedule it, check it against everyone's timetables, midterms and CS3216 deadlines. If it clashes, move it to Tuesday 13 October or to the following week. Don't launch on a day when two of the four of us have exams.

---

## What we verified (24 September 2026)
...
| Video | YouTube only, full URL, no short links. About 53% of Product of the Day winners since 2021 had a video | Launch Guide |
...
## Checklist from T−14 to T+2

### T−14 · Wed 30 Sep: decide and set up
- [ ] Read `readiness.md` together and assign an owner to every must-fix item. The launch date depends on items 1 to 5.
...
- [ ] Choose the launch date and write it in the team calendar in **both** time zones: "Wed 14 Oct, 00:01 PDT = 15:01 SGT".

### T−12 · Fri 2 Oct: product-side fixes start
- [ ] Pricing section: Plus marked "Coming soon", and the "Try Plus free for 7 days" button removed (readiness #1).
- [ ] Draft the privacy policy and terms (readiness #2).
- [ ] Set up a support email and put it on the landing page FAQ and footer, replacing the GitHub link (readiness #3).
...
- [ ] Coming Soon pages no longer exist. Create the Linguini **product forum thread** instead, with one post: what we're building, one gallery image, and "we're launching on 14 Oct, and we'd love your feedback on the demo". Link the demo.
- [ ] Soft teaser on our own channels (a personal LinkedIn or X post): "We're launching Linguini on Product Hunt on 14 Oct." No upvote asks.

### T−7 · Wed 7 Oct: draft on Product Hunt
- [ ] Create the **draft** in the Pre-Launch Dashboard and paste everything from `submission.md`.
...
- [ ] Load check: can the AI vision provider handle a spike, say 200 sessions in an hour? Check rate limits, spending caps and billing alerts on the provider account (readiness #5).
- [ ] Turn on error alerts, such as Vercel logs and an uptime ping on the app and API, so D gets notified.
- [ ] Add analytics (Vercel Web Analytics or similar) to the landing page, so we can measure what's listed below.

### T−3 · Sun 11 Oct: warm the list
...
- [ ] Send a heads-up to that list only: "We launch on Wednesday at 3 pm SGT. Here's the demo if you want a sneak peek." Include the demo link and no upvote ask.
- [ ] Finalise `social-posts.md` and fill in the date.

### T−1 · Tue 13 Oct: freeze
...
### T0 · Wed 14 Oct: launch (see the hour-by-hour plan below)

### T+1 · Thu 15 Oct
- [ ] Keep replying until the Product Hunt day ends at 2:59 pm SGT. Late comments still deserve answers.
- [ ] 3:00 pm SGT: post a short thank-you with what we learned. Don't mention rank.
- [ ] Put every piece of feedback into one list, tagged bug / feature / copy / question.

### T+2 · Fri 16 Oct
- [ ] Fix the top 3 bugs raised on launch day, then reply in those threads to say they're fixed.
- [ ] Write a retro using the numbers below. What would we change for the next launch? (Product Hunt allows new launches "for significant product iterations".)
- [ ] Email everyone who left an email or a detailed comment and thank them personally.
...
## Hour by hour (SGT, with PT)

| SGT (Wed 14 → Thu 15) | PT (Tue 13 → Wed 14) | Who | What |
|---|---|---|---|
| **14:30 Wed** | 23:30 Tue | A, C, D | On a call. D confirms app, API, landing and demo are up and the vision provider has credit. |
| **15:01** | 00:01 | A | Launch goes live. **Post the first comment within the minute.** Open the page logged out and check the gallery, links and video. |
...
| 23:30–03:00 Thu | 08:30–12:00 | B, D | US morning is the busiest part of the Product Hunt day. B replies to every comment. D fixes anything that blocks sign-up or a session. |
...
| **14:59 Thu** | 23:59 Wed | All | Product Hunt day ends. |
| 15:00 | 00:00 | A | Thank-you comment on the launch thread saying what we heard and what we'll fix first. |

## Replying to comments

General rules: reply to every comment. Thank people for specific things. Say "you're right" when they're right. Link to things instead of pasting long text. Never argue with a critic. Never ask for an upvote, not even jokingly.
...
**"What about the privacy of my photos?"** *(Before launch, confirm the AI provider's data terms and link the privacy policy. See readiness #2.)*
...
> Right now it's Spanish and French, taught from English. We're a small team and want those two to be really good before we add more. Which language would you want? Replies here count as votes for our roadmap.

*(The landing FAQ says "Italian is next on the list". Only repeat that if the team has committed to it.)*

**"Is the AI accurate?"**
> Mostly, but not always. That's why you review the words before every session: you untick anything wrong and tap the photo to add something it missed. The AI suggests, and you decide. Every word card shows the article, gender, meaning, IPA and audio, so you can check the details as well. If you catch a wrong word or translation, please tell us here with the word, and we'll fix it.

**"What does it cost?"**
> Linguini is free today: one new photo session a day, the journal, all the games and streaks, with no card needed. We plan a paid Plus tier later (unlimited sessions, pronunciation feedback, review mode). It isn't live yet, and nothing on Product Hunt today is behind a paywall.

*(Only use the "one session a day" line if the limit is actually enforced in the app. Otherwise say "free while we're in early launch".)*
...
## Do and don't (based on Product Hunt's guidelines)
