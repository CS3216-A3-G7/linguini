...
The banners are drawn by `banners.py` (run `python3 launch/media/banners.py`), which reuses the helpers in `render.py` and pins each photo's words at the positions in `landing/data/scenes.ts`. It also writes web JPEGs to `landing/public/blog/launch/` for the launch blog post. The launch date is the `LAUNCH` constant at the top.
