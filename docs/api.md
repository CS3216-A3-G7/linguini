...
| `POST` | `/media/upload-url` | `CreateUploadUrlResponse` | Allocate an owned image key and signed upload URL. | Current user; returns `200` because the route declares no explicit status code, unlike `/media/confirm-upload`, which declares `201`. |
