...
Content capture is off by default and is opted into by setting
`AI_OBSERVABILITY_CAPTURE_CONTENT=true`. When false, learner text, model
responses, image bytes, base64 payloads, keys, and signed URLs are not attached.
When enabled, feature generation
