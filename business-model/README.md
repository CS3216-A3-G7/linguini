...
| --- | --- | --- |
| Beta, now to the Product Hunt launch | D7 second-lesson retention and substantive feedback reports (targets are in [`launch/plan.md`](../launch/plan.md#launch-goals-and-definitions)) | A new learner product fails on retention before it fails on price. |
...
- **Opening condition:** Founding Plus opens only when Plus checkout, entitlements and cancellation work end to end, as the [launch go/no-go](../launch/plan.md#go--no-go-at-t1) requires. Until then, promote the free beta and collect interest without taking payment.
...
Regenerate with `python3 business-model/model/cost_model.py && python3 business-model/media/render.py` (requires Pillow). The renderer reuses the helpers in `launch/media/render.py` and the landing site's brand assets. Photographs are credited in `landing/public/photos/credits.json`; confirm their rights before posting, as the launch media kit notes.
