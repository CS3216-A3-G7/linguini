...
Run from any directory with: python3 business-model/media/render.py
Requires Pillow. Reads business-model/model/unit-economics.json (run
cost_model.py first) and reuses the drawing helpers in launch/media/render.py.
Outputs go to business-model/media/export/.
"""
...
MODEL = json.loads((ROOT / "business-model/model/unit-economics.json").read_text())

sys.dont_write_bytecode = True  # keep launch/media free of __pycache__
_spec = importlib.util.spec_from_file_location("launch_render", ROOT / "launch/media/render.py")
...
def chart_session_cost():
    ...
    c, d = chart_frame(
        ...
        "Model: business-model/model/cost_model.py · expected tokens, 10% retry allowance · "
