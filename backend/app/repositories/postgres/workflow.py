...
class PostgresWorkflowRepository:
    ...
    def create(self, request):
        with self.transaction() as c:
            ...
            if active:
                if active["scene_media_asset_id"] == request.media_asset_id:
                    return self._detail(c, parse_session(active))
                raise ActiveSessionExistsError(
                    "Finish or discard your current session before starting a new one.",
                    active["id"],
                )
            session = Session(
